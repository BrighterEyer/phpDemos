<?php
	...
	$db = database();
	$rows = $db->where(array('id'=>$id))->select("article_category");
	$row = $rows[0];
	//判断是否提交了数据
	...
	if($_POST){
		...
		//插入数据
		$rst = $db->where(array('id'=>$id))->update('article_category',$data);
		...
		<div class="container">
			<form class="" method="post">
				<div class="form-group">
					...
					<input type="text" name="name" id="name" value="<?php echo $row['name']?>" class="form-control" placeholder="请输入分类名称" />
				</div>
				...
				<div class="form-group">
					...
					<input type="text" name="sug" value="<?php echo $row['sug']?>" class="form-control" placeholder="请输入英文名称" />
				</div>
